...
func (conf *FileConfig) Check() error {
	...
	// Make sure that either CA path or the CA pin is set and not both.
	if conf.CAPath != "" && conf.CAPin != "" {
		return trace.BadParameter("CA path or pin can be set, not both")
	}
...
type Global struct {
	...
	InsecureSkipCAVerification bool `yaml:"insecure_skip_ca_verification"`
}
