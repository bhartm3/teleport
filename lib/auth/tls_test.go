...
// TestRegisterInsecure checks to make sure that both CA path and CA pin are
// ignored when registering with an Auth Server.
func (s *TLSSuite) TestRegisterInsecure(c *check.C) {
	...
	// Insecure registration even with a invalid pin should work.
	...
	// Insecure registering even with a invalid CA path should work.
	_, err = Register(RegisterParams{
		Servers: []utils.NetAddr{utils.FromAddr(s.server.Addr())},
		Token:   token,
		ID: IdentityID{
...
func (s *TLSSuite) TestRegisterCAPath(c *check.C) {
	...
	// Attempt to register with invalid CA path, should fail.
	_, err = Register(RegisterParams{
		Servers: []utils.NetAddr{utils.FromAddr(s.server.Addr())},
		Token:   token,
		ID: IdentityID{
